...
abstract class CWG_Instock_Mailer {
	...
	public function format_html_message() {
		ob_start();
		if($this->get_subject()=="Product Status : Instock"){
			wc_get_template( 'emails/instock-email-header.php' );
			echo do_shortcode( $this->get_message() );
			wc_get_template( 'emails/instock-email-footer.php' );
		}else {
			wc_get_template( 'emails/oos-email-header.php' );
			echo do_shortcode( $this->get_message() );
			wc_get_template( 'emails/oos-email-footer.php' );
		}
		return ob_get_clean();
	}
