...
class Wt_woocommerce_invoice_addon_Admin {
		...
		if ( ! in_array( 'wt_main_advanced', $tab_content_arr ) && isset( $_GET['page'] ) && 'wf_woocommerce_packing_list' === $_GET['page'] ) {
			...
			$params                  = array(
				...
				'msgs'                    => array(
					...
					'settings_error'                      => sprintf( __( 'Unable to update settings due to an internal error. %1$s To troubleshoot please click %2$s here. %3$s', 'wt_woocommerce_invoice_addon' ), '<br />', '<a href="https://www.webtoffee.com/how-to-fix-the-unable-to-save-settings-issue/" target="_blank">', '</a>' ),
	...
	public function default_settings( $settings, $base_id ) {
		...
		if ( $base_id === 'main' ) {
			...
			$settings['wt_pklist_dokan_compatibility_enable'] = 'No';
		}
		return $settings;
	}
	...
	public function single_checkbox_fields( $settings, $base_id, $tab_name ) {
		...
		if ( 'main' === $base_id && 'wt_main_advanced' === $tab_name ) {
			$settings['wt_main_advanced'] = array(
				...
				'wt_pklist_dokan_compatibility_enable' => 'No',
			);
		}
		return $settings;
	}
	...
	/**
	 * Function to update the changes in database working like updation hook
	 */
	public function do_update_to_current_version() {
		$wt_pklist_ver = get_option( 'wt_pklist_ipc_version' );
		if ( false === $wt_pklist_ver || empty( $wt_pklist_ver ) || ( ! empty( $wt_pklist_ver ) && version_compare( trim( $wt_pklist_ver ), WT_PKLIST_INVOICE_ADDON_VERSION ) < 0 ) ) {
			include_once WT_PKLIST_INVOICE_ADDON_PLUGIN_PATH . 'includes/class-wt_woocommerce_invoice_addon-activator.php';
			Wt_woocommerce_invoice_addon_Activator::do_update();
			update_option( 'wt_pklist_ipc_version', WT_PKLIST_INVOICE_ADDON_VERSION );
		}
	}
	...
	public function additional_checkout_fields_in_order_detail_page( $order ) {
		if ( apply_filters( 'wt_pklist_show_additional_checkout_fields_in_order_detail_page', true ) ) {
			$checkout_fields_arr = $this->add_checkout_fields( array( 'billing' => array() ) );
			...
			if ( $checkout_fields_arr && is_array( $checkout_fields_arr ) && isset( $checkout_fields_arr['billing'] ) && is_array( $checkout_fields_arr['billing'] ) ) {
				...
				foreach ( $checkout_fields_arr['billing'] as $field_key => $field_vl ) {
					$val = Wt_Pklist_Common_Ipc::get_order_meta( $order_id, '_' . $field_key, true );
					if ( $hide_empty_fields ) {
						if ( '' !== $val ) {
							echo '<p><strong>' . wp_kses_post( $field_vl['label'] ) . ':</strong> ' . wp_kses_post( $val ) . '</p>';
						}
					} else {
						echo '<p><strong>' . wp_kses_post( $field_vl['label'] ) . ':</strong> ' . wp_kses_post( $val ) . '</p>';
					}
			...
			add_filter(
				'wt_pklist_show_additional_checkout_fields_in_order_detail_page',
				function ( $show ) {
					return false;
				}
			);
