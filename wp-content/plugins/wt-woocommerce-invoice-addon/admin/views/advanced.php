...
<div class="wf-tab-content" data-id="<?php echo $target_id;?>">
    <form method="post" class="wf_settings_form wf_general_settings_form">
            ...
            <tbody>
                <?php 
                    ...
                    if(class_exists('Wf_Woocommerce_Packing_List_Pro_Common_Func')){
                        ...
                        $account_doc_types['invoice'] = __("Invoice","wt_woocommerce_invoice_addon");
                    }
                    if(class_exists('Wf_Woocommerce_Packing_List_Pro_Common_Func_SDD')){
                        ...
                        $account_doc_types['dispatchlabel'] = __("Dispatch label","wt_woocommerce_invoice_addon");
                    }
                    $pack_help = "";
                    ...
                    if(class_exists('Wf_Woocommerce_Packing_List_Pro_Common_Func_PKL')){
                        $packaging_doc_types['picklist'] = __("Pick list","wt_woocommerce_invoice_addon");
                    }
                    if(class_exists('Wf_Woocommerce_Packing_List_Pro_Common_Func_PI')){
                        $account_doc_types['proformainvoice'] = __("Proforma invoice","wt_woocommerce_invoice_addon");
                    }
                    $dokan_support_docs = array_merge($account_doc_types,$packaging_doc_types);
                    $dokan_help = "";
                    if(!empty($dokan_support_docs)){
                        sort($dokan_support_docs);
                        $dokan_help = sprintf(__('Note: This option is applicable on %1s only','wt_woocommerce_invoice_addon'),implode(', ', $dokan_support_docs));
                    }
        ...
        <table class="wf-form-table">
            <tbody>
                    <?php
                    $dokan_compatibility['advanced_option']['wt_pklist_dokan_compatibility_enable'] = array(
                            'type'  => 'wt_single_checkbox',
                            'label' => __("Compatibility with Dokan plugin","wt_woocommerce_invoice_addon"),
                            'id'    => 'wt_pklist_dokan_compatibility_enable',
                            'name'  => 'wt_pklist_dokan_compatibility_enable',
                            'value' => "Yes",
                            'checkbox_fields' => array('Yes'=> __("Enable","wt_woocommerce_invoice_addon")),
                            'class' => "wt_pklist_dokan_compatibility_enable",
                            'col'   => 3,
                            'help_text' => __("The vendor`s store address will be used as From address in all the document types instead of general store address","wt_woocommerce_invoice_addon").'<br><b>'.$dokan_help.'</b>',
                    );
                    $dokan_compatibility = Wf_Woocommerce_Packing_List::add_fields_to_settings($dokan_compatibility);
                    if(class_exists('WT_Form_Field_Builder_PRO_Documents')){
                        $Form_builder = new WT_Form_Field_Builder_PRO_Documents();
                    }else{
                        $Form_builder = new WT_Form_Field_Builder();
                    }
                    foreach($dokan_compatibility as $dokan){
                        $Form_builder->generate_form_fields($dokan);
                    }
                ?>
            </tbody>
        </table>
