...
class WT_Form_Field_Builder_PRO_Documents extends WT_Form_Field_Builder{
	...
	/**
	 * To show the multi language compatibility option
	 *
	 * @since 1.1.0
	 * @param array $args
	 ...
	 * @return string
	 */
	public function multi_language_option($args,$base_id){
		
		extract( $this->verify_the_fields( $args ) );
		$result			= Wf_Woocommerce_Packing_List::get_option( $name, $base_id );
		$result			= is_string( $result ) ? stripslashes( $result ) : $result;
		$radio_fields_help_text = (isset($radio_fields_help_text) && !empty($radio_fields_help_text) && is_array($radio_fields_help_text)) ? $radio_fields_help_text : array();
		$td_sytle 		= "";
		$radio_opt_name = $name;
		...
		foreach( $radio_fields as $radio_key => $radio_label ) {
			...
			if( isset( $radio_fields_help_text[$radio_key] ) && !empty( $radio_fields_help_text[$radio_key] ) ) {
				$html .= sprintf( '<div style="display:flex;" class="wf_form_help wf_form_help_option_wise">%1$s</div>', wp_kses_post( $radio_fields_help_text[$radio_key] ) );
			}
