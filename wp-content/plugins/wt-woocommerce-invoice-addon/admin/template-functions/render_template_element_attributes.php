...
if(!class_exists('Wf_Woocommerce_Packing_List_Template_Render_IPC')){
class Wf_Woocommerce_Packing_List_Template_Render_IPC
{
    ...
    public static function get_row_key($column_key){
        if(false !== strpos($column_key,'custom_st_row_')){
    ...
    public static function get_column_key($column_key){
        if(false !== strpos($column_key,'custom_pt_col_')){
    ...
    public static function render_unit_price($col_val_source_details){
        ...
        if(isset($column_list_options_value[$columns_key_real]['tax-type'])){
            $p_tax_type_name = $column_list_options_value[$columns_key_real]['tax-type'];
            if($p_tax_type_name === "incl_tax"){
                $p_incl_tax = true;
            }
        }
        if(isset($column_list_options_value[$columns_key_real]['discount-type'])){
            $discount_type = $column_list_options_value[$columns_key_real]['discount-type'];
        }
    ...
    public static function render_total_tax($col_val_source_details,$template_type=""){
        ...
        if(isset($column_list_options_value[$columns_key_real]['discount-type']) && "after_discount" === $column_list_options_value[$columns_key_real]['discount-type']){
            $discount_type = "after_discount";
            $item_tax_subtotal	= (isset($item_taxes['total']) ? $item_taxes['total'] : array());
        }
        if(isset($column_list_options_value[$columns_key_real]['total-tax-display-option'])){
            $tax_display_option = "" !== trim($column_list_options_value[$columns_key_real]['total-tax-display-option']) ? $column_list_options_value[$columns_key_real]['total-tax-display-option']: $tax_display_option;
        }
    ...
    public static function render_tax_items_column($col_val_source_details,$template_type=""){
        ...
        // render the tax items column key
        $tax_item_col_key = '';
        if(strpos($columns_key_real,'_wtpdf_')){
            $tax_col_key_arr = explode('_wtpdf_',$columns_key_real);
            $tax_item_col_default_key = 'custom_pt_col_tax_items_wtpdf_';
            if(isset($tax_col_key_arr[1])){
                $tax_item_col_key = $tax_item_col_default_key.$tax_col_key_arr[1];
            }
        }
        $tax_item_col_key   = !empty($tax_item_col_key) ? $tax_item_col_key : 'tax_items';

        if(isset($column_list_options_value[$tax_item_col_key]['discount-type']) && "after_discount" === $column_list_options_value[$tax_item_col_key]['discount-type']){
            $item_tax_subtotal	= (isset($item_taxes['total']) ? $item_taxes['total'] : array());
        }

        $tax_info_arr 		= explode("individual_tax_",$columns_key_real);
        $tax_display_type 	= "amount";
        $req_col_key_arr = explode('_wtpdftaxitem_',$columns_key_real);
        $columns_key_real = isset($req_col_key_arr[1]) ? $req_col_key_arr[1] : $columns_key_real;
        if(isset($column_list_options_value[$tax_item_col_key]['ind-tax-display-option'])){
            $tax_display_type = $column_list_options_value[$tax_item_col_key]['ind-tax-display-option'];
        ...
        $tax_id = explode("_wtpdftaxitem_",$tax_id);
        $tax_id = $tax_id[0];
        ...
        $tax_val 	= apply_filters('wf_pklist_alter_item_individual_tax', $tax_val, $template_type, $tax_id, $order_item, $order,$tax_display_type);
    ...
    public static function process_all_values($order,$template_type){
        ...
        foreach($order_items as $order_item){
            ...
            /**
             * @since 1.1.0 - [Fix] - Division by zero when quantity becomes zero
             */
            $divided_by = (0 !== $quantity ) ? $quantity : 1;
            
            $unit_coupon_amount_it   += (float)$line_discount_it/abs($divided_by);
            $unit_coupon_amount_et   += (float)$line_discount_et/abs($divided_by);
     ...
     */
    public static function render_discount_price($col_val_source_details){
        extract($col_val_source_details);
        $show_after_refund = apply_filters('wt_pklist_show_details_after_refund_'.$template_type,false,$template_type);
        $discount = (float)$order_item->get_subtotal() - (float)$order_item->get_total(); 
        if(isset($column_list_options_value[$columns_key_real]['tax-type'])){
            $p_tax_type_name = $column_list_options_value[$columns_key_real]['tax-type'];
            if("incl_tax" === $p_tax_type_name){
                $discount += ((float)$order_item->get_subtotal_tax() - (float)$order_item->get_total_tax()); 
            }
        }

        $item_qty_refunded = 0;
        if($show_after_refund){
            $item_qty_refunded = (float)$order->get_qty_refunded_for_item( $order_item->get_id() );
        }

        if(isset($item['quantity']) && isset($document_type) && $document_type === "package"){
            $discount_unit_price = $discount / max( 1, (abs( $order_item['qty'] ) - abs($item_qty_refunded)) );
            $discount = (float)$item['quantity'] * (float)$discount_unit_price;
        }
        return $discount;
    }
     ...
     * @return string|int|float
     */
    public static function render_regular_unit_price($col_val_source_details){
        extract($col_val_source_details);
        $p_incl_tax = false;
        $discount_type = "before_discount";
        if(isset($column_list_options_value[$columns_key_real]['tax-type'])){
        ...
        if($_product){
            if ( !$_product->is_on_sale() ) {
                ...
                return $_product->get_regular_price();
            }
        }else{
            return Wf_Woocommerce_Packing_List_Order_Func_Pro::get_unit_price($order_item,$order,$p_incl_tax,$discount_type);
        }
     ...
     * @return string|int|float
     */
    public static function render_total_regular_price($col_val_source_details){
        extract($col_val_source_details);
        if($_product){
            if( !$_product->is_on_sale() ){
                return self::render_total_price($col_val_source_details);
            }
            else
            {
                $show_after_refund = apply_filters('wt_pklist_show_details_after_refund_'.$template_type,false,$template_type);
                ...
                $r_price = $_product->get_regular_price(); 
                $tot_price = $r_price * max( 1, (abs( $order_item['qty'] ) - abs($item_qty_refunded)) );
                if(isset($document_type) && isset($item))
                {
                    $unit_price = $tot_price / max( 1, (abs( $order_item['qty'] ) - abs($item_qty_refunded)) );
                    $tot_price = (float)$item['quantity'] * (float)$unit_price;
                }
                return $tot_price;
            }
        }
