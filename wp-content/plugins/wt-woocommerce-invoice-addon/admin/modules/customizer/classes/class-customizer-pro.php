...
class Wf_Woocommerce_Packing_List_Customizer_Ipc_PRO{
    ...
    public static function caseInsensitiveKeySortASC($a, $b) {
        return strcasecmp($a, $b);
    }
    
    public static function caseInsensitiveKeySortDESC($a, $b) {
    ...
    */
    public static function set_total_weight($find_replace, $template_type, $html, $order=null)
    {
        ...
        if(false !== strpos($html,'[wfte_weight]') && !isset($find_replace['[wfte_weight]'])) //if total weight placeholder exists then only do the process, If already added then skip
        {
            ...
            {
                $find_replace['[wfte_weight]']  = empty( $total_weight ) ? '' : $total_weight.' '.get_option('woocommerce_weight_unit');
            }
        }
        return $find_replace;
    }
    ...
    public static function set_package_weight_and_count_info($find_replace,$template_type,$order,$box_packing,$order_package){
        ...
        if(!is_null($order))
        {
            if(is_array($order_package) && !empty($order_package)){
                ...
                $weight_data    = empty( $package_weight ) ? '' : $package_weight.' '.get_option('woocommerce_weight_unit');
        ...
        if(!is_null($order))
        {
            ...
            if(!isset($find_replace['[wfte_extra_fields]'])) /* check already added */
            {
                ...
                foreach ($unset_keys as $unset_key) {
                    if (($index_key = array_search($unset_key, $default_fields)) !== false) {
                        unset($default_fields[$index_key]);
                    }
                }
    ...
    */
    private static function get_default_order_fields($key,$template_type,$order,$order_id,$wc_version)
    {
        ...
        }elseif("vat" === $key || 'vat_number' === $key || 'eu_vat_number' === $key || 'aelia_vat' === $key)
        {
            $vat_fields = array('vat','vat_number','eu_vat_number','aelia_vat');
    ...
    */
    ...
    private static function dummy_product_row($columns_list_arr,$column_display_option_attr=array())
    {
        ...
        $dummy_vals=array(
            'serial_no'=>'1',
            'image'=>Wf_Woocommerce_Packing_List_Template_Render_IPC::generate_product_image_column_data(0,0,0,$img_style),
        ...
        foreach($columns_list_arr as $columns_key=>$columns_value)
        {
            ...
            $column_id  = Wf_Woocommerce_Packing_List_Template_Render_IPC::get_column_key($columns_key);
            ...
            {
                $col_key_arr = explode('_wtpdf_',$columns_key);
                $columns_key = isset($col_key_arr[0]) ? $col_key_arr[0] : $columns_key;
            ...
            elseif("tax_items" === $columns_key || "tax_items" === Wf_Woocommerce_Packing_List_Template_Render_IPC::get_column_key($columns_key))
            {
            ...
            }elseif("tax" === $columns_key || "tax" === Wf_Woocommerce_Packing_List_Template_Render_IPC::get_column_key($columns_key)){
            ...
            }elseif("product" === $columns_key || "product" === Wf_Woocommerce_Packing_List_Template_Render_IPC::get_column_key($columns_key)){
            ...
            elseif(0 === strpos($columns_key, 'custom_pt_col_'))
            {
                $col_key_arr = explode('_wtpdf_',$columns_key);
                $columns_key = isset($col_key_arr[0]) ? $col_key_arr[0] : $columns_key;
                $column_val = ucfirst(str_replace('custom_pt_col_', '', $columns_key));
                $column_val = "{".$column_val."}";
            }
            $column_val = apply_filters('wt_pklist_alter_dummy_col_value',$column_val,$columns_key,$columns_list_arr,$column_display_option_attr);
    ...
    */
    ...
    {   
        ...
        $find_replace['[wfte_shipping_address]']    = 'Shipping address name <br>20 Maple Avenue <br>San Pedro, California, 90731 <br>United States (US)';
