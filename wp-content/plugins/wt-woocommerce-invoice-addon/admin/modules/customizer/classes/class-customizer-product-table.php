...
trait Wf_Woocommerce_Packing_List_Customizer_Product_table_Ipc_Pro {
	...
	*/
			...
			if ( '' !== $th_html ) {
				...
				foreach ( $th_html_arr as $th_single_html ) {
					...
					if ( '' !== $th_single_html ) {
						...
						if ( 'tax' === $col_key || '-tax' === $col_key || 'tax' === Wf_Woocommerce_Packing_List_Template_Render_IPC::get_column_key( $col_key ) ) {
						...
						} elseif ( 'tax_items' === $col_key || '-tax_items' === $col_key || 'tax_items' === Wf_Woocommerce_Packing_List_Template_Render_IPC::get_column_key( $col_key ) ) {
							if ( ! is_null( $order ) ) {
								...
								if ( true === $show_individual_tax_column || 'Yes' === $show_individual_tax_column ) {
									...
									if ( false === $individual_tax_column_display_option ) {
									...
									*/
									...
									foreach ( $tax_items as $tax_item ) {
										...
										$tax_id_rate_only = $tax_id_rate_only . '_wtpdftaxitem_' . $col_key;
										$tax_id           = $tax_id_prefix . $tax_id;
										$tax_id           = $tax_id . '_wtpdftaxitem_' . $col_key;
										...
										*/
										if ( 'separate-column' === $individual_tax_column_display_option && ! $show_individual_tax_rate_column_after_amount_column ) {
											self::prepare_tax_item_column_html( $col_key, $tax_id_rate_only, $tax_rate_only_column_label, $th_single_html, $columns_list_arr, $column_list_options, $extra_table_col_opt, $template_type, $tax_item, $individual_tax_column_config, $order );
										}

										self::prepare_tax_item_column_html( $col_key, $tax_id, $tax_label, $th_single_html, $columns_list_arr, $column_list_options, $extra_table_col_opt, $template_type, $tax_item, $individual_tax_column_config, $order );
								...
								$tax_item_col_head_string = 'wfte_product_table_tax_item_column_label_wtpdftaxitem_';
								$th_single_html           = preg_replace( '/\[' . preg_quote( $tax_item_col_head_string ) . '[^\]]*\]/i', 'Tax items', $th_single_html );
	...
	private static function prepare_tax_item_column_html( $col_key, $tax_id, $tax_label, $th_single_html, &$columns_list_arr, &$column_list_options, $extra_table_col_opt, $template_type, $tax_item, $individual_tax_column_config, $order ) {
		...
		if ( false !== strpos( $th_single_html, '[wfte_product_table_tax_item_column_label_wtpdftaxitem_' . $col_key . ']' ) ) {
			$col_html = str_replace( '[wfte_product_table_tax_item_column_label_wtpdftaxitem_' . $col_key . ']', $tax_label, $th_single_html );
		}
	...
	public static function set_extra_charge_fields( $find_replace, $template_type, $html, $order = null ) {
		...
		if ( ! is_null( $order ) ) {
			...
			$predefined_placeholders_summary_table = array(
				...
				'wfte_product_table_additional_info',
			...
			$order_details = Wf_Woocommerce_Packing_List_Template_Render_IPC::process_all_values( $order, $template_type );
			...
			if ( ! empty( $summary_table_row_arr ) ) {
				foreach ( $summary_table_row_arr as $this_st_row_key => $this_st_row_val ) {
					if ( 'wfte_product_table_subtotal' === $this_st_row_key || 'subtotal' === Wf_Woocommerce_Packing_List_Template_Render_IPC::get_row_key( $this_st_row_key ) ) {
						$incl_tax_text = '';
						...
						$sub_total                                    = Wf_Woocommerce_Packing_List_Template_Render_IPC::process_summary_table_subtotal_row( $this_st_row_key, $this_st_row_val, $order_items, $order, $template_type, $order_details );
					...
					if ( 'wfte_product_table_fee' === $this_st_row_key || 'fee' === Wf_Woocommerce_Packing_List_Order_Func_Pro::get_row_key( $this_st_row_key ) ) {
						...
						if ( ! empty( $fee_details ) ) {
							...
							foreach ( $fee_details as $fee => $fee_detail ) {
								$fee_detail_html   = Wf_Woocommerce_Packing_List_Admin::wf_display_price( $user_currency, $order, $fee_total_amount ) . ' via ' . $fee_detail['name'];
					...
					$refunded_tax_details = Wf_Woocommerce_Packing_List_Template_Render_IPC::wt_get_total_tax_refunded( $order );
					...
					if ( 'wfte_product_table_additional_info' === $this_st_row_key || 'additional_info' === Wf_Woocommerce_Packing_List_Order_Func_Pro::get_row_key( $this_st_row_key ) ) {
						$additional_row_arr = array();
						$order_totals_arr = $order->get_order_item_totals();

						/**
						 * @since 1.1.0 - yith gift card compatibility
						 */
						if(!empty($order_totals_arr) && isset($order_totals_arr['gift_cards'])){
							$gf_price = Wt_Pklist_Common_Ipc::get_yith_applied_coupon_total($order);
							$gf_code	= Wt_Pklist_Common_Ipc::get_yith_applied_coupon_codes($order);
					...
					if ( 'wfte_product_table_tax_item' === $this_st_row_key || 'tax_item' === Wf_Woocommerce_Packing_List_Order_Func_Pro::get_row_key( $this_st_row_key ) ) {
						...
						if ( preg_match( '/<[^>]*data-row-type\s*=\s*"[^"]*\b' . $this_st_row_key . '\b[^"]*"[^>]*>(.*?)<\/tr>/s', $html, $tax_items_match ) ) {
							$tax_items_row_html = isset( $tax_items_match[0] ) ? $tax_items_match[0] : '';
						} elseif ( preg_match( '/<[^>]*data-row-type\s*=\s*"[^"]*\bwfte_product_table_tax_item\b[^"]*"[^>]*>(.*?)<\/tr>/s', $html, $tax_items_match ) ) {
							$tax_items_row_html = isset( $tax_items_match[0] ) ? $tax_items_match[0] : '';
						} elseif ( preg_match( '/<[^>]*data-row-type\s*=\s*"[^"]*\bwfte_tax_items\b[^"]*"[^>]*>(.*?)<\/tr>/s', $html, $tax_items_match ) ) {
							$tax_items_row_html = isset( $tax_items_match[0] ) ? $tax_items_match[0] : '';
						}

						if ( ! empty( $shipping_tax_items ) ) { 
							foreach ( $shipping_tax_items as $tax_item_id => $tax_item ) {
								...
								if ( 'wfte_product_table_tax_item' === $this_st_row_key ) {
								...
								} elseif ( false !== strpos( $this_st_row_key, 'wfte_payment_summary_table_custom_st_row_tax_item_wtpdf' ) ) {
									$req_row_key_arr = explode( 'wfte_payment_summary_table_', $this_st_row_key );
									if ( isset( $req_row_key_arr[1] ) ) {
										$req_row_key     = '[wfte_product_table_tax_item_label_wtpdfsttaxitem_' . $req_row_key_arr[1] . ']';
										$req_row_val_key = '[' . $this_st_row_key . ']';
										$tax_items_html .= str_replace( array( $req_row_key, $req_row_val_key ), array( $tax_label, $tax_amount ), $tax_items_row_html );
										if ( '' !== $tax_items_row_html && isset( $tax_items_match[0] ) ) {
											$find_replace[ $tax_items_match[0] ] = $tax_items_html; //replace tax items
										}
									}
							...
							if(!empty($tax_items_match)){
								$find_replace[ $tax_items_match[0] ] = $tax_items_html;
							}
						}
					}
					...
					if ( 'wfte_product_table_total_weight' === $this_st_row_key || 'total_weight' === Wf_Woocommerce_Packing_List_Template_Render_IPC::get_row_key( $this_st_row_key ) ) {
					...
					if ( 'wfte_product_table_total_qty' === $this_st_row_key || 'total_qty' === Wf_Woocommerce_Packing_List_Template_Render_IPC::get_row_key( $this_st_row_key ) ) {
			...
			if ( self::get_summary_table_custom_order_meta_placeholders( $html, $custom_order_meta_datas ) ) {
				foreach ( $custom_order_meta_datas as $custom_order_meta_item ) {
					$full_row_placeholder = $custom_order_meta_item[0];
					// render the custom meta key from the row key
					$actual_row_meta = $custom_order_meta_item[1];
					if ( false !== strpos( $custom_order_meta_item[1], '_wtpdf_' ) ) {
						$pm_arr          = explode( '_wtpdf_', $actual_row_meta );
						$actual_row_meta = isset( $pm_arr[0] ) ? $pm_arr[0] : $custom_order_meta_item;
					}
					$order_meta_value                      = Wt_Pklist_Common_Ipc::get_order_meta( $order_id, $actual_row_meta, true );
					$find_replace[ $full_row_placeholder ] = self::process_meta_value( $order_meta_value );
				}
			}
		} else {
			/**
			...
			if ( self::get_summary_table_custom_order_meta_placeholders( $html, $custom_order_meta_datas ) ) {
				foreach ( $custom_order_meta_datas as $custom_order_meta_item ) {
					...
					$find_replace[ $custom_order_meta_item[0] ] = $actual_row_meta;
				}
			}
		}
		return $find_replace;
	...
	*/
	public static function generate_package_product_table_product_column_html( $wc_version, $the_options, $order, $template_type, $_product, $item, $columns_list_arr, $column_list_options_value ) {
		...
		foreach ( $columns_list_arr as $columns_key => $columns_value ) {
			...
			$col_val_source_details['columns_key_real'] = $columns_key;
			if ( 0 === strpos( $columns_key, 'default_column_' ) ) {
				$columns_key = str_replace( 'default_column_', '', $columns_key );
			}
			$columns_key                           = Wf_Woocommerce_Packing_List_Template_Render_IPC::get_column_key( $columns_key );
			...
			if ( 'serial_no' === $columns_key || '-serial_no' === $columns_key ) {
				...
				$column_data = Wf_Woocommerce_Packing_List_Template_Render_IPC::generate_product_image_column_data( $product_id, $variation_id, $parent_id, $column_list_options_value['image'] );
			} elseif ( 'sku' === $columns_key || '-sku' === $columns_key ) {
				$column_data = ! empty( $_product ) && method_exists( $_product, 'get_sku' ) ? $_product->get_sku() : $item['sku'];
				...
				if ( $_product ) {
					...
					if ( class_exists( 'Wf_Woocommerce_Packing_List_Template_Render_Adc' ) ) {
						...
						if ( isset( $the_options[ 'wf_' . $template_type . '_product_meta_fields' ] ) && is_array( $the_options[ 'wf_' . $template_type . '_product_meta_fields' ] ) && count( $the_options[ 'wf_' . $template_type . '_product_meta_fields' ] ) > 0 ) {
							...
							$addional_product_meta = Wf_Woocommerce_Packing_List_Template_Render_IPC::process_p_meta_product_name_column_package_doc( $addional_product_meta, $col_val_source_details );
						}
					}
					...
					*/
					...
					if ( class_exists( 'Wf_Woocommerce_Packing_List_Template_Render_Adc' ) ) {
						...
						$product_attr = Wf_Woocommerce_Packing_List_Template_Render_IPC::process_p_attr_product_name_column_package_doc( $product_attr, $col_val_source_details );
					}
				...
				$item_price          = Wf_Woocommerce_Packing_List_Template_Render_IPC::render_unit_price( $col_val_source_details );
				$item_price          = apply_filters( 'wf_pklist_alter_package_item_price', $item_price, $template_type, $_product, $item, $order );
				$item_price_formated = Wf_Woocommerce_Packing_List_Admin::wf_display_price( $user_currency, $order, $item_price );
				$column_data         = apply_filters( 'wf_pklist_alter_package_item_price_formated', $item_price_formated, $template_type, $item_price, $_product, $item, $order );
			} elseif ( 'regular_price' === $columns_key || '-regular_price' === $columns_key ) {
				// $item_price  = Wf_Woocommerce_Packing_List_Admin::wf_convert_to_user_currency($item['price'],$user_currency,$order);
				$item_price          = Wf_Woocommerce_Packing_List_Template_Render_IPC::render_regular_unit_price( $col_val_source_details );
				$item_price          = apply_filters( 'wf_pklist_alter_package_item_regular_price', $item_price, $template_type, $_product, $item, $order );
				$item_price_formated = Wf_Woocommerce_Packing_List_Admin::wf_display_price( $user_currency, $order, $item_price );
				$column_data         = apply_filters( 'wf_pklist_alter_package_item_regular_price_formated', $item_price_formated, $template_type, $item_price, $_product, $item, $order );
				...
				$product_total          = Wf_Woocommerce_Packing_List_Template_Render_IPC::render_total_price( $col_val_source_details );
			...
			} elseif ( 'total_regular_price' === $columns_key || '-total_regular_price' === $columns_key ) {
				$product_total          = Wf_Woocommerce_Packing_List_Template_Render_IPC::render_total_regular_price( $col_val_source_details );
				$total_price            = apply_filters( 'wf_pklist_alter_package_item_total_regular_price', $product_total, $template_type, $_product, $item, $order );
				$product_total_formated = Wf_Woocommerce_Packing_List_Admin::wf_display_price( $user_currency, $order, $total_price );
				$column_data            = apply_filters( 'wf_pklist_alter_package_item_total_regular_price_formated', $product_total_formated, $template_type, $product_total, $_product, $item, $order );

			} elseif ( false !== strpos( $columns_key, 'individual_tax_' ) || 'tax_items' === $columns_key ) {
				$column_data = Wf_Woocommerce_Packing_List_Template_Render_IPC::render_tax_items_column( $col_val_source_details );
			} elseif ( 'tax' === $columns_key || '-tax' === $columns_key ) {
				$column_data = Wf_Woocommerce_Packing_List_Template_Render_IPC::render_total_tax( $col_val_source_details );
			} elseif ( 'discount' === $columns_key || '-discount' === $columns_key ) {
				$discount                     = Wf_Woocommerce_Packing_List_Template_Render_IPC::render_discount_price( $col_val_source_details );
				$discount                     = apply_filters( 'wf_pklist_alter_package_item_discount_price', $discount, $template_type, $_product, $order_item, $order );
				$item_discount_price_formated = Wf_Woocommerce_Packing_List_Admin::wf_display_price( $user_currency, $order, $discount );
				$column_data                  = apply_filters( 'wf_pklist_alter_package_item_discount_price_formated', $item_discount_price_formated, $template_type, $discount, $_product, $item, $order );
			...
			} else //custom column by user
			{
				...
				$column_data = apply_filters( 'wt_pklist_add_package_product_table_additional_column_value', $column_data, $template_type, $columns_key_real, $_product, $item, $order, $column_list_options_value );
			}
			...
			if ( 'single_packing' === $package_type && ( 'Yes' === $category_wise_split || 'Yes' === $order_wise_split ) ) {
				...
				foreach ( $order_package as $id => $item ) {
					...
					if ( 'Yes' === $category_wise_split ) {
						...
						{
							if ( isset( $item['extra_meta_details'] ) && isset( $item['extra_meta_details']['_product_id'] ) ) {
				...
				$item_arr = apply_filters(
					'wf_pklist_alter_package_grouped_order_items',
					$item_arr,
					array(
						'order'    => $order_wise_split,
						'category' => $category_wise_split,
					),
					$order_package,
					$template_type,
					$order
				...
				if ( 'Yes' === $order_wise_split && 'Yes' === $category_wise_split ) {
					foreach ( $item_arr as $key => $val_arr ) {
						/**
						 * @since 1.2.0 - Added category name sorting when working with group by cateory option - accounting document
						 */
						if( !empty( $val_arr ) && is_array( $val_arr ) ) {
							if( "desc" === apply_filters('wt_pklist_alter_category_sort_order', 'asc', $template_type, $order) ) {
								uksort($val_arr, array('Wf_Woocommerce_Packing_List_Customizer_Ipc_PRO', 'caseInsensitiveKeySortDESC'));
							}else{
								uksort($val_arr, array('Wf_Woocommerce_Packing_List_Customizer_Ipc_PRO', 'caseInsensitiveKeySortASC'));
							}
						}
					...
					if( !empty( $item_arr ) && is_array( $item_arr ) ) {
						if( "desc" === apply_filters('wt_pklist_alter_category_sort_order', 'asc', $template_type, $order) ) {
							uksort($item_arr, array('Wf_Woocommerce_Packing_List_Customizer_Ipc_PRO', 'caseInsensitiveKeySortDESC'));
						}else{
							uksort($item_arr, array('Wf_Woocommerce_Packing_List_Customizer_Ipc_PRO', 'caseInsensitiveKeySortASC'));
						}
					}
				...
				$serial_no = 1;
				if ( 'single_packing' === $package_type && 'picklist' === $template_type ) {
					$item_arr = array();
					foreach ( $order_package as $id => $item ) {
						$product_id = ( '' !== $item['variation_id'] ? $item['variation_id'] : $item['id'] );

						if ( isset( $item_arr[ $product_id ] ) ) {
	...
	public static function is_bundle_type( &$is_bundle, $order_item_id ) {
		...
		*/
		...
		if ( isset( $meta_data_arr['_bundled_items'] ) || isset( $meta_data_arr['_cartstamp'] ) ) {
			$is_bundle = 1; //parent
			if ( isset( $meta_data_arr['_bundle_cart_key'] ) ) {
				$bundle_info_arr['_bundle_cart_key'] = $meta_data_arr['_bundle_cart_key'];

			} elseif ( isset( $meta_data_arr['_yith_bundle_cart_key'] ) ) {
					...
					if ( 'main' === $bundle_display_option ) {
						...
						if ( isset( $bundle_info_arr['_bundled_by'] ) && '' !== $bundle_info_arr['_bundled_by'] ) {
							...
							if ( ! isset( $bundle_child_arr[ $bundle_info_arr['_bundled_by'] ] ) ) {
								if ( '' !== $previous_sort_key && isset( $out[ $previous_sort_key ] ) ) {
									if ( '' === $out[ $previous_sort_key ][2]['_bundle_cart_key'] ) {
		...
		foreach ( $out as $v ) {
			...
			if ( 'Yes' !== $category_wise_split ) {
				if ( isset( $v[2] ) && isset( $v[2]['_bundle_cart_key'] )
					...
					&& isset( $bundle_child_arr[ $v[2]['_bundle_cart_key'] ] ) ) {
	...
	*/
			...
			foreach ( $item_arr as $item_key => $items ) {
				...
				foreach ( $items as $order_item_id => $order_item ) {
					...
					if ( 'creditnote' === $template_type ) {
						...
						if ( !empty($order_item_id) && class_exists('WT_WC_Order_Item_Product_IPC') && class_exists('WC_Order_Item_Product') ) {
							$order_item = new WT_WC_Order_Item_Product_IPC( $order_item_id );
						}
					...
					if ( $_product ) {
						...
						foreach ( $columns_list_arr as $columns_key => $columns_value ) {
							$columns_key_real                           = $columns_key; /* backup */
							$col_val_source_details['columns_key_real'] = $columns_key_real;
							if ( 0 === strpos( $columns_key, 'default_column_' ) ) {
								$columns_key = str_replace( 'default_column_', '', $columns_key );
							}
							$columns_key                           = Wf_Woocommerce_Packing_List_Template_Render_IPC::get_column_key( $columns_key );
							...
							if ( 'serial_no' === $columns_key || '-serial_no' === $columns_key ) {
								...
								if ( class_exists( 'Wf_Woocommerce_Packing_List_Template_Render_Adc' ) ) {
									...
									$meta_data_formated_arr = Wf_Woocommerce_Packing_List_Template_Render_IPC::process_p_meta_product_name_column( $meta_data_formated_arr, $col_val_source_details );
								}
								...
								*/
								...
								if ( class_exists( 'Wf_Woocommerce_Packing_List_Template_Render_Adc' ) ) {
									...
									$product_attr_formated_arr = Wf_Woocommerce_Packing_List_Template_Render_IPC::process_p_attr_product_name_column( $product_attr_formated_arr, $col_val_source_details );
								}
								...
								*/
								...
								$item_price          = Wf_Woocommerce_Packing_List_Template_Render_IPC::render_regular_unit_price( $col_val_source_details );
								$item_price          = apply_filters( 'wf_pklist_alter_item_regular_price', $item_price, $template_type, $_product, $order_item, $order );
								$item_price_formated = Wf_Woocommerce_Packing_List_Admin::wf_display_price( $user_currency, $order, $item_price );
								$column_data         = apply_filters( 'wf_pklist_alter_item_regular_price_formated', $item_price_formated, $template_type, $item_price, $_product, $order_item, $order );
							} elseif ( 'discount' === $columns_key || '-discount' === $columns_key ) {
								$discount                     = Wf_Woocommerce_Packing_List_Template_Render_IPC::render_discount_price( $col_val_source_details );
								$discount                     = apply_filters( 'wf_pklist_alter_item_discount_price', $discount, $template_type, $_product, $order_item, $order );
								$item_discount_price_formated = Wf_Woocommerce_Packing_List_Admin::wf_display_price( $user_currency, $order, $discount );
								$column_data                  = apply_filters( 'wf_pklist_alter_item_discount_price_formated', $item_discount_price_formated, $template_type, $discount, $_product, $order_item, $order );
							} elseif ( false !== strpos( $columns_key, 'individual_tax_' ) || 'tax_items' === $columns_key ) {
								$column_data = Wf_Woocommerce_Packing_List_Template_Render_IPC::render_tax_items_column( $col_val_source_details );
							} elseif ( 'tax' === $columns_key || '-tax' === $columns_key ) {
								$column_data = Wf_Woocommerce_Packing_List_Template_Render_IPC::render_total_tax( $col_val_source_details );
								...
								$product_total          = Wf_Woocommerce_Packing_List_Template_Render_IPC::render_total_regular_price( $col_val_source_details );
								$product_total          = apply_filters( 'wf_pklist_alter_item_total_regular_price', $product_total, $template_type, $_product, $order_item, $order, $incl_tax );
								$product_total_formated = Wf_Woocommerce_Packing_List_Admin::wf_display_price( $user_currency, $order, $product_total );
								$column_data            = apply_filters( 'wf_pklist_alter_item_total_regular_price_formated', $product_total_formated, $template_type, $product_total, $_product, $order_item, $order, $incl_tax );
							} elseif ( 'category' === $columns_key || '-category' === $columns_key ) {
								if ( ! empty( $parent_id ) ) {
									$product_id = $parent_id;
								}
							...
							} else //custom column by user
							{
								...
								$column_data = apply_filters( 'wt_pklist_add_product_table_additional_column_value', $column_data, $template_type, $columns_key_real, $_product, $order_item, $order, $column_list_options_value );
							}
							$product_row_columns[ $columns_key_real ] = $column_data;
						}
						...
						foreach ( $columns_list_arr as $columns_key => $columns_value ) {
							...
							if ( 'serial_no' === $columns_key || '-serial_no' === $columns_key ) {
								...
								$discount                     = Wf_Woocommerce_Packing_List_Template_Render_IPC::render_discount_price( $col_val_source_details );
								$discount                     = apply_filters( 'wf_pklist_alter_item_discount_price_deleted_product', $discount, $template_type, $_product, $order_item, $order );
								$item_discount_price_formated = Wf_Woocommerce_Packing_List_Admin::wf_display_price( $user_currency, $order, $discount );
								$column_data                  = $item_discount_price_formated;
							} else {
								$column_data = '';
								if ( ! self::set_custom_meta_table_col_data( $column_data, $columns_key_real, $product_id, $order_item_id, $order_item ) ) {
									$column_data = apply_filters( 'wf_pklist_product_table_additional_column_val', $column_data, $template_type, $columns_key_real, $_product, $order_item, $order );
	...
	public static function set_custom_meta_table_col_data( &$column_data, $columns_key, $product_id, $order_item_id, $order_item ) {
		if ( 0 === strpos( $columns_key, 'custom_product_meta_' ) ) {
			...
			if ( false !== strpos( $product_meta_key, '_wtpdf_' ) ) {
				$pm_arr           = explode( '_wtpdf_', $product_meta_key );
				$product_meta_key = isset( $pm_arr[0] ) ? $pm_arr[0] : $product_meta_key;
			}
			...
			if ( false !== strpos( $order_item_meta_key, '_wtpdf_' ) ) {
				$pm_arr              = explode( '_wtpdf_', $order_item_meta_key );
				$order_item_meta_key = isset( $pm_arr[0] ) ? $pm_arr[0] : $order_item_meta_key;
			}
			...
			return true;
		} elseif ( 0 === strpos( $columns_key, 'custom_product_attr_' ) ) {
			$product_attr_key = str_replace( 'custom_product_attr_', '', $columns_key );
			if ( false !== strpos( $product_attr_key, '_wtpdf_' ) ) {
				$pm_arr           = explode( '_wtpdf_', $product_attr_key );
				$product_attr_key = isset( $pm_arr[0] ) ? $pm_arr[0] : $product_attr_key;
			}
			$product_attr_value = '';
			$col_product        = wc_get_product( $product_id );
			if ( $col_product && ( $col_product->is_type( 'simple' ) || $col_product->is_type( 'variable' ) ) ) {
				$product_attr_value = $col_product->get_attribute( $product_attr_key );
			}

			$column_data = self::process_meta_value( $product_attr_value );
			if ( isset( $col_product ) ) {
				unset( $col_product );
			}
			return true;
		}
	...
	*/
	public static function get_total_table_columms_enabled( $columns_list_arr ) {
		...
		foreach ( $columns_list_arr as $key => $value ) {
			if ( ('-' !== substr( $key, 0, 1 )) && (false === strpos($key, '-individual')) ) {
				$total++;
			}
		}
		return $total;
