...
class Wt_woocommerce_invoice_addon {
	...
	private function load_libraries() {
		 ...
		 * core plugin.
		 */
		require_once plugin_dir_path( __DIR__ ) . 'includes/class-wt_woocommerce_invoice_addon-loader.php';

		/**
		 * The class responsible for defining internationalization functionality
		 * of the plugin.
		 */
		require_once plugin_dir_path( __DIR__ ) . 'includes/class-wt_woocommerce_invoice_addon-i18n.php';
	...
	private function include_plugin_files() {
		 ...
		 */
		require_once plugin_dir_path( __DIR__ ) . 'admin/class-wt_woocommerce_invoice_addon-admin.php';

		/**
		 * The class responsible for defining all actions that occur in the public-facing
		 * side of the site.
		 */
		require_once plugin_dir_path( __DIR__ ) . 'public/class-wt_woocommerce_invoice_addon-public.php';
		...
		require_once plugin_dir_path( __DIR__ ) . 'includes/class-wf-woocommerce-packing-list-pay-later-payment.php';
		
		require_once plugin_dir_path( __DIR__ ) . 'admin/views/_form_field_generator_pro.php';

		if ( class_exists( 'Wf_Woocommerce_Packing_List_Customizer' ) && ! class_exists( 'Wf_Woocommerce_Packing_List_Customizer_Ipc_PRO' ) ) {
			require_once plugin_dir_path( __DIR__ ) . 'admin/modules/customizer/customizer.php';
		}

		if ( ! class_exists( 'Wf_Woocommerce_Packing_List_Template_Load' ) ) {
			require_once plugin_dir_path( __DIR__ ) . 'admin/template-functions/load_template_element_attributes.php';
		}

		if ( ! class_exists( 'Wf_Woocommerce_Packing_List_Template_Render_IPC' ) ) {
			require_once plugin_dir_path( __DIR__ ) . 'admin/template-functions/render_template_element_attributes.php';
		}

		if ( ! class_exists( 'Wf_Woocommerce_Packing_List_Order_Func_Pro' ) ) {
			require_once plugin_dir_path( __DIR__ ) . 'admin/template-functions/class-wf-woocommerce-packing-list-order-func-pro.php';
		}

		if ( ! class_exists( 'Wf_Woocommerce_Packing_List_Template_Load_Epo_Tm' ) ) {
			require_once plugin_dir_path( __DIR__ ) . 'admin/template-functions/compatibility_epo_theme_complete.php';
		}

		if ( ! class_exists( 'Wt_Pdf_Blocks' ) ) {
			require_once plugin_dir_path( __DIR__ ) . 'blocks/wt-pdf-blocks.php';
		}
		...
		if ( ! class_exists( 'Wf_Woocommerce_Packing_List_Pro_Common_Func' ) ) {
			require_once plugin_dir_path( __DIR__ ) . 'includes/class-wt_woocoommerce_packing_list_pro_func.php';
		...
		if ( ! $license_tab_exist ) {
			require_once plugin_dir_path( __DIR__ ) . 'includes/licence_manager/licence_manager.php';
	...
	private function define_admin_hooks() {
		...
		$this->plugin_admin->do_update_to_current_version();
		...
		$this->loader->add_action( 'woocommerce_admin_order_data_after_billing_address', $this->plugin_admin, 'additional_checkout_fields_in_order_detail_page' ); /* Show additional checkout fields on order details page */
