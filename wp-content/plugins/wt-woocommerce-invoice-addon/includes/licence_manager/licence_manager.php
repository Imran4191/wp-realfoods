...
if ( ! class_exists( 'Wf_Woocommerce_Packing_List_Licence_Manager' ) ) {
	...
	class Wf_Woocommerce_Packing_List_Licence_Manager {
		...
		public $my_account_url   = 'https://www.webtoffee.com/my-account';
		...
		public function __construct() {
			...
			$this->main_plugin_slug = 'wt-woocommerce-invoice-addon'; // main file name.
			$this->products         = array();
		...
		*/
		public function licence_tabhead( $arr ) {
			...
			if ( $status ) {
				...
				if ( 0 === count( $licence_statuses ) || in_array( 'inactive', $licence_statuses ) || in_array( '', $licence_statuses ) ) {
					$status = false;
				}
			}
			...
			$arr['wt-licence'] = array( __( 'License', 'wt_woocommerce_invoice_addon' ), $activate_icon );
			return $arr;
		}
		...
		*/
		public function licence_content() {
			...
			$params = array(
				...
				'msgs'      => array(
					'key_mandatory'      => __( 'Please enter license key', 'wt_woocommerce_invoice_addon' ),
					...
					'unable_to_fetch'    => __( 'Unable to fetch license details', 'wt_woocommerce_invoice_addon' ),
					'no_licence_details' => __( 'No license details found.', 'wt_woocommerce_invoice_addon' ),
		...
		*/
		...
		/**
		 * Get the license data of plugins
		 *
		...
		private function get_licence_data( $product_slug = '' ) {
			...
			if ( '' !== $product_slug ) {
				$licence_data = (array) get_option( $product_slug . '_licence_data', false );
			} else {
				$licence_data = array();
		...
		public function activate( $out ) {
			...
			if ( 0 === $er && '' !== $licence_key ) {
				...
				foreach ( $licence_data as $product_slug => $licence_info ) {
					if ( $product_slug == $licence_product ) {
			...
			if ( 0 === $er ) {
				...
				if ( is_wp_error( $response ) ) {
					...
					if ( isset( $response_arr['success'] ) && true === $response_arr['success'] ) {
						$licence_data  = array(
							'key'         => $licence_key,
		...
		*/
		...
		public function add_licence_status_message( $plugin_file, $plugin_data, $status ) {
			?>
		<tr class="plugin-update-tr installer-plugin-update-tr wt-pklist-addons-plugin-notice-tr">
			<td colspan="4" class="plugin-update colspanchange">
				<div class="update-message notice inline">
					<p><?php echo sprintf( __( 'The plugin license is not activated. You will not receive compatibility and security updates if the plugin license is not activated. %1$sActivate now%2$s', 'wt_woocommerce_invoice_addon' ), '<a href="' . esc_url( admin_url( 'admin.php?page=wf_woocommerce_packing_list#wt-licence' ) ) . '" target="_blank">', '</a>' ); ?></p>
				</div>
			</td>
		</tr>
			<?php
		...
		public function plugins_page_scripts() {
			?>
		<style type="text/css">
			.wt-pklist-addons-plugin-notice-tr p:before{ content: "\f534"; }
		</style>
		<script type="text/javascript">
			jQuery(document).ready(function(){
				jQuery('.wt-pklist-addons-plugin-notice-tr').each(function(){
		...
		public function add_to_edd_plugin_updator() {
			...
			include_once plugin_dir_path( __FILE__ ) . 'classes/EDD_WT_Pklist_Addons_Plugin_Updater.php';
			...
			foreach ( $this->products as $product_slug => $product ) {
				...
				if ( $status && function_exists( 'is_plugin_active' ) && is_plugin_active( $plugin_base_path ) ) { // Plugin and licence are active
					...
					new Wf_Woocommerce_Packing_List\Licence_Manager\EDD_WT_Pklist_Addons_Plugin_Updater(
						$this->api_url,
