...
if( !class_exists( 'Wf_Woocommerce_Packing_List_Licence_Manager_Edd' )) {
	class Wf_Woocommerce_Packing_List_Licence_Manager_Edd
	{
		...
		*/
		...
		/**
		 * To get the status of the license data
		 *
		...
		public function check_status( $licence_data, $response_arr )
		{
			...
			if ( isset( $response_arr['success'] ) ) {
				if ( true === $response_arr['success'] ) {
					if ( isset( $response_arr['license'] ) && "valid" === $response_arr['license'] ) {
						$new_status	= 'active';
					} else {
						$new_status	= 'inactive';
					}
