    ...
    class Wf_Woocommerce_Packing_List_Box_packing
    {
        ...
        public function wf_pklist_create_order_single_package($order) {
            ...
            foreach ($order_items as $id => $item) 
            {
                ...
                if($product) 
                {
                    $skip_display_arr   = Wf_Woocommerce_Packing_List::get_option('wt_pklist_skip_vitual_backorder_items',$module_id);
                    if(!empty($skip_display_arr) && is_array($skip_display_arr)){
                        if(in_array('virtual_items',$skip_display_arr) && $product->is_virtual()){
                            continue;
                        }
                    }

                    if(!empty($skip_display_arr) && is_array($skip_display_arr)){
                        if(in_array('backorder_items',$skip_display_arr) && $product->is_on_backorder()){
                            continue;
                        }
                    }
                    ...
                    $is_bundle          = false;
                    $bundle_info_arr    = Wf_Woocommerce_Packing_List_Customizer_Ipc_PRO::is_bundle_type($is_bundle, $id);
                    if(1 === absint($is_bundle) && "sub" === Wf_Woocommerce_Packing_List::get_option('bundled_product_display_option',$module_id)){
                        continue;
                    }if(2 === absint($is_bundle) && "main" === Wf_Woocommerce_Packing_List::get_option('bundled_product_display_option',$module_id)){
                        continue;
                    }
                    if (WC()->version < '2.7.0') {
        ...
        {   
            ...
            if (!empty($items)) {
                ...
                foreach ($package['contents'] as $item_id => $values) {
                    $is_bundle  = false;
                    $bundle_info_arr    = Wf_Woocommerce_Packing_List_Customizer_Ipc_PRO::is_bundle_type($is_bundle, $values['order_item_id']);
                    if(1 === absint($is_bundle) && "sub" === Wf_Woocommerce_Packing_List::get_option('bundled_product_display_option',$module_id)){
                        continue;
                    }if(2 === absint($is_bundle) && "main" === Wf_Woocommerce_Packing_List::get_option('bundled_product_display_option',$module_id)){
                        continue;
                    ...
                    if ($values['data']) {
                        if (!$values['data']->needs_shipping()) {
                            if(false === $is_bundle){
                                continue;
                            }elseif(1 === absint($is_bundle) || 2 === absint($is_bundle)){
                                return $this->wf_pklist_create_order_indvidual_item_package($order);
                                break;
                            }
                        }
        ...
        public function wf_pklist_create_order_indvidual_item_package($order) {
            ...
            foreach ($order_items as $id => $item) {
                ...
                if($product) 
                {
                    ...
                    for ($item_count = 0; $item_count < $item->get_quantity(); $item_count++) {
                        $packinglist_package[][] = array(
                            'sku' => $item->get_meta( '_sku', true ),
                            'name' => $item->get_name(),
                            'type' => $item->get_type(),
