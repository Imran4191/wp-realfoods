...
class Wf_Woocommerce_Packing_List_Invoice_Pro 
{
    ...
    public function __construct()
    {
        ...
        */
        ...
        add_filter('wt_pklist_alter_document_language_for_'.$this->module_base, array($this,'alter_document_language'), 10, 3);
    }
    ...
    */
    ...
    /**
     * Add the fields for invoice settings page
     *
     * @since 1.1.0 - Added an option to set the document language
    ...
    public function add_remove_fields_from_settings($settings,$target_id,$template_type,$base_id){
        ...
        if($base_id === $this->module_id){
            ...
            if("general" === $target_id){
                ...
                if(isset($settings['invoice_general_general'])){
                    $document_language = array(
                        'wt_pklist_document_language' => array(
                            'type' => 'multi_language_option',
                            'label' => __("Generate invoice in","wt_woocommerce_invoice_addon"),
                            'id' => '',
                            ...
                            'radio_fields_help_text' => array(
                                'site'=>__("In multilingual sites, invoice language may differ from the default one set in WordPress Settings > General > Site Language, depending on customer`s  choice via the site`s language switcher.","wt_woocommerce_invoice_addon"),
                                'customer'=>__("To generate a invoice in the customer`s language, ensure that the plugin has the corresponding language file for the selected language. If not, add it to use the functionality.","wt_woocommerce_invoice_addon")
                            ), 
                            'col' => 3,
                            'tooltip' => true,
                    ...
                    $settings['invoice_general_general'] = Wt_woocommerce_invoice_addon_Admin::wt_add_array_element_to_position($settings['invoice_general_general'],$document_language,'wf_woocommerce_invoice_show_print_button');
                }
            }
    ...
    */
    ...
    public function wf_allow_payment_for_order_status($statuses, $order){
        ...
        if("wf_pay_later" === $order_payment_method) {
        ...
        if(!empty($pay_later_role_access)) {
            ...
            foreach($pay_later_role_access as $role) //checking access
            {
                if (in_array($role, $user->roles)) { //any of the role present in the role array then restrict the access for the payment gateway
                    unset($available_gateways['wf_pay_later']);
                    break;
                }
            }
        }
    ...
    */
     ...
     * @return string
     */
    public function alter_document_language($lang,$template_type,$order_id) {
        if($this->module_base === $template_type && !empty($order_id)){
            $document_language_to = Wf_Woocommerce_Packing_List::get_option('wt_pklist_document_language',$this->module_id);
