...
class Wf_Woocommerce_Packing_List_Packinglist_Pro 
{
    ...
    public function __construct()
    {
        ...
        $this->print_btn_label	= __('Print packing slip', 'wt_woocommerce_invoice_addon');
		$this->download_btn_label	= __('Download packing slip', 'wt_woocommerce_invoice_addon');
        ...
        if(is_array($vl) && in_array('wfte_seperate_email',$vl)){
            ...
            // email customizer compatibility
            add_filter('wt_decorator_email_template_types',array($this,'email_customizer_compatibiity_email_types'),10,1);
            add_filter('wt_decorator_email_type_class_name_array',array($this,'email_customizer_compatibiity_email_class_names'),10,1);
            add_filter('kadence_woomail_email_types',array($this,'email_customizer_compatibiity_email_types'),10,1);
            add_filter('kadence_woomail_email_type_class_name_array',array($this,'email_customizer_compatibiity_email_class_names'),10,1);
        }
        ...
        if($show_print_button_pages){
            ...
            if(is_array($show_print_button_arr) && in_array('order_details',$show_print_button_arr)){
                ...
                if((in_array('wc-'.$order->get_status(),$generate_invoice_for)))
                {
                    ...
                    Wf_Woocommerce_Packing_List::generate_print_button_for_user($order,$order_id,'print_packinglist',$print_btn_label);
                }
            }
        }
        return $html;
    ...
    */
    ...
    public function default_settings($settings,$base_id)
    {
        ...
        {      
            ...
            $settings['wt_pklist_skip_vitual_backorder_items'] = array();
            $settings['wt_pklist_document_language']                   = 'site';
        }
        return $settings;
    }
    ...
    public function multi_checkbox_fields($settings,$base_id,$tab_name){
        if($base_id === $this->module_id){
            ...
            $settings['wt_packinglist_general']['wt_pklist_skip_vitual_backorder_items'] = array();
    ...
    public function alter_validation_rule($arr, $base_id)
    {
        if($base_id === $this->module_id)
        {
            ...
            $arr['wt_pklist_skip_vitual_backorder_items'] = array('type'=>'text_arr');
        ...
        {                    
            ...
            { 
                ...
                if(!$attachment_file)
                {
                    ...
                    $html   = $this->generate_order_template($order_ids, $pdf_name,'','email_attachment');
                    if( !empty( $html ) ) {
    ...
    */    
    ...
    public function generate_order_template($orders,$page_title,$html="",$action="")
    {
        ...
        if("" !== $html)
        {
            ...
            foreach ($orders as $order_id)
            {
                ...
                $lang   = self::alter_document_language($lang,$template_type,$order_id);
            ...
            if($no_item_in_order === count($orders)){
                if( 'email_attachment' === $action ) {
                    return '';
                }else{
    ...
    /**
     * Compatibility with Email customizer plugins
     * To add the packing slip email type to the email customizer
     * @since 1.0.6
     * @param array $types
     * @return array
     */
    public function email_customizer_compatibiity_email_types($types){
        $types = array_merge( $types, array(
            'wf_woocommerce_packing_list_packinglist_email'     => __( 'Packing slip', 'wt_woocommerce_invoice_addon' ),
        ) );
        return $types;
    }
    ...
    /**
     * Compatibility with Email customizer plugins
     * To add the packing slip email class name to the email customizer
     * @since 1.0.6
     * @param array $class_names
     * @return array
     */
    public function email_customizer_compatibiity_email_class_names($class_names){
        $class_names['wf_woocommerce_packing_list_packinglist_email'] = 'Wf_Woocommerce_Packing_List_Packinglist_Email';
        return $class_names;
    }

    /**
     * Get the document language as per the settings
     *
     * @since 1.1.0
     * @param string $lang
     ...
     * @return string
     */
    public static function alter_document_language($lang,$template_type,$order_id) {
        if(!empty($order_id)){
            $module_id              = self::$module_id_static;
            $document_language_to   = Wf_Woocommerce_Packing_List::get_option('wt_pklist_document_language',$module_id);
            if(!empty($document_language_to)){
                switch ($document_language_to) {
                    case 'site':
                        $lang   = !empty(get_option('WPLANG')) ? get_option('WPLANG') : get_locale();
                        return $lang;
                        break;
                    case 'customer':
                        $clang  = Wt_Pklist_Common_Ipc::get_order_meta($order_id,'wt_pklist_order_language',true);
                        $lang   = !empty($clang) ? $clang : $lang;
                        return $lang;
                    case 'current_user':
                        $user_id        = get_current_user_id();
                        $user_locale    = get_user_locale( $user_id );
                        $lang           = !empty($user_locale) ? $user_locale : $lang;
                        return $lang;
                    default:
                        return $lang;
                        break;
