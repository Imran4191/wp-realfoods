...
class Wf_Woocommerce_Packing_List_Creditnote_Pro 
{
    ...
    public function default_settings($settings,$base_id)
    {
        if($base_id === $this->module_id)
        {
            $settings = array(
                ...
                'woocommerce_wf_custom_pdf_name' => '[prefix][order_no]', 
				'woocommerce_wf_custom_pdf_name_prefix' => 'Creditnote_',
                'wt_pklist_document_language'   => 'site',
            );
        }
        return $settings;
    }
    ...
    */
    ...
    {   
        ...
        add_filter('wf_pklist_alter_item_individual_tax',array($this,'alter_item_individual_tax_column'),1,6);
    ...
    * 
    */
    public function alter_item_individual_tax_column($tax_val, $template_type, $tax_id, $order_item, $order, $tax_display_type=""){
        $wc_version             = WC()->version;
        $order_id               = $wc_version<'2.7.0' ? $order->id : $order->get_id();
        $user_currency          = Wt_Pklist_Common_Ipc::get_order_meta($order_id,'currency',true);
        $new_tax_val_formatted  = $tax_val;
        $full_refunded          = Wt_Pklist_Common_Ipc::is_fully_refunded($order);
        if("rate" === trim($tax_display_type)){
            return '';   
        }
        if(false === $full_refunded && "rate" != $tax_display_type){
            $refunded_item_id   = $order_item->this_refund_item_id;
            $new_tax_val        = 0;
            $refunded_item      = new WC_Order_Item_Product($refunded_item_id);
    ...
    */    
    ...
    public function generate_order_template($orders,&$page_title)
    {
        ...
        $out            = '';
        $wf_invoice_pdf_name_format = Wf_Woocommerce_Packing_List::get_option('woocommerce_wf_custom_pdf_name', $this->module_id);
		$wf_invoice_pdf_name_prefix = Wf_Woocommerce_Packing_List::get_option('woocommerce_wf_custom_pdf_name_prefix', $this->module_id);

        if("[prefix][invoice_no]" === $wf_invoice_pdf_name_format){
            $set_credit_note_no = true;
        }
        if("" !== $html)
        {
            $order_inc          = 0;
            foreach($orders as $order_id)
            {
                ...
                foreach($all_refund_orders as $refund_order){
                    ...
                    $crt_no .=Wt_Pklist_Common::get_order_meta($refund_id,'wf_creditnote_number',true);
                    $page++;
                }
                ...
                {
                    if(1 === $number_of_orders && "[prefix][order_no]" === $wf_invoice_pdf_name_format){
                        $page_title = $wf_invoice_pdf_name_prefix.$order->get_order_number();
                    }
                }
            }

            if(isset($set_credit_note_no) && true === $set_credit_note_no && !empty($crt_no)){
                $page_title = $wf_invoice_pdf_name_prefix.$crt_no;
            }
