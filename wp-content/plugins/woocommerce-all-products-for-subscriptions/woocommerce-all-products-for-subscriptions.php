...
class WCS_ATT extends WCS_ATT_Abstract_Module {
	...
	public function plugins_loaded() {
		...
		if ( class_exists( 'WC_Subscriptions' ) ) {
			...
			if ( ! defined( 'WCS_INIT_TIMESTAMP' ) || ! WC_Payments_Features::is_wcpay_subscriptions_enabled() ) {
				if ( version_compare( WCPAY_VERSION_NUMBER, self::REQ_WCPAY_VERSION ) < 0 ) {
					...
					$notice = sprintf( __( 'All Products for Woo Subscriptions requires Subscriptions to be enabled in the <strong>WooCommerce Payments</strong> <a href="%1$s" target="_blank">settings</a>.', 'woocommerce-all-products-for-subscriptions' ), self::get_resource_url( 'wcpay-settings' ) );
				}
			}
		} else {
		...
		if ( ! function_exists( 'phpversion' ) || version_compare( phpversion(), '7.0.0', '<' ) ) {
			$notice = sprintf( __( 'All Products for Woo Subscriptions requires at least PHP <strong>%1$s</strong>. Learn <a href="%2$s">how to update PHP</a>.', 'woocommerce-all-products-for-subscriptions' ), '7.0.0', 'https://woocommerce.com/document/how-to-update-your-php-version/' );
			require_once( WCS_ATT_ABSPATH . 'includes/admin/class-wcs-att-admin-notices.php' );
			WCS_ATT_Admin_Notices::add_notice( $notice, 'error' );
			return false;
		}
