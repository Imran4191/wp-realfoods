...
<div class="um <?php echo esc_attr( $this->get_class( $mode ) ); ?> um-<?php echo esc_attr( $form_id ); ?>">
	<div class="um-form">
		<form method="post" action="">
			<h3><span>Reset</span> Your Password</h3>
						...
						<div>
							<?php
								printf(
									...
									'<p>' . esc_html__( 'If an account matching the provided details exists, we will send a password reset link. Please check your inbox.', 'ultimate-member' ) . '</p>',
								);

								printf(
									/* translators: %s: home url */
									esc_html__( '%s', 'ultimate-member' ),
									'<a href="' . esc_url( home_url() ) . '">' . esc_html__( 'Return to Home', 'ultimate-member' ) . '</a>'
								);
							?>
						</div>
					</div>
			...
			<?php } else { ?>
				...
				<?php } else { ?>
					<div class="um-field um-field-block um-field-type_block">
						<div class="um-field-block">
							<div>
								<?php esc_html_e( 'Please enter your email address below to receive a password reset link.' ); ?>
							</div>
						</div>
					</div>
				<?php }
				...
				foreach ( $fields as $key => $data ) {
					if ($key === 'username_b') {
						echo '<label for="username_b">' . $label_text . '</label>';
