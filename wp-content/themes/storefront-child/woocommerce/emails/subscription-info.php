...
<h2 style="margin-top: 10px; color:#08456b"><?php esc_html_e( 'Subscription information', 'woocommerce-subscriptions' ); ?></h2>
<table class="td" cellspacing="0" cellpadding="6" style="width: 100%; color:#08456b; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif; margin-bottom: 0.5em;" border="1">
	<thead>
		<tr>
	...
	<?php foreach ( $subscriptions as $subscription ) : ?>
		...
		<tr>
			<td class="td" scope="row" style="text-align:left;color:#08456b;"><a style="color:#08456b;" href="<?php echo esc_url( ( $is_admin_email ) ? wcs_get_edit_post_link( $subscription->get_id() ) : $subscription->get_view_order_url() ); ?>"><?php echo sprintf( esc_html_x( '#%s', 'subscription number in email table. (eg: #106)', 'woocommerce-subscriptions' ), esc_html( $subscription->get_order_number() ) ); ?></a></td>
...
<?php if ( $has_automatic_renewal && ! $is_admin_email && $subscription->get_time( 'next_payment' ) > 0 ) {
	...
	), '<a style="color:#08456b;" href="' . $my_account_url . '">', '</a>' ) ) );
}?>
</div>
