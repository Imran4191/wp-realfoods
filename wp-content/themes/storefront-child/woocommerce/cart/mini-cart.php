...
if ( empty( $cart_items ) ) { ?>
	<div class="woocommerce-mini-cart__empty-message"><?php esc_attr_e( 'Your shopping cart is empty.', 'elementor-pro' ); ?></div>
<?php } else { ?>
	<div class="elementor-menu-cart__header-details">
		<div class="items-total">
			<strong class="count"><?php echo count($cart_items); ?></strong>
			<span>item<?php if(count($cart_items)>1) echo 's'; ?></span>
		</div>
		<div class="elementor-menu-cart__subtotal">
			<span class="label"><?php echo esc_html__( 'Cart Subtotal', 'woocommerce' ); // phpcs:ignore WordPress.WP.I18n ?> :</span>
			<strong class="amount-total"><?php echo WC()->cart->get_cart_subtotal(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></strong>
		</div>
	</div>
	<div class="elementor-menu-cart__header-buttons">
		<a href="<?php echo esc_url( wc_get_checkout_url() ); ?>" class="elementor-button elementor-button--checkout elementor-size-md">
			<span class="elementor-button-text"><?php echo esc_html__( 'Go To Checkout', 'woocommerce' ); // phpcs:ignore WordPress.WP.I18n ?></span>
		</a>
	</div>
	<?php if ( ! function_exists( 'elementor_pro_render_mini_cart_item' ) ) {
...
?>
	...
	<div class="elementor-menu-cart__footer-buttons">
		<a href="<?php echo esc_url( wc_get_cart_url() ); ?>" class="elementor-button elementor-button--view-cart elementor-size-md">
			<span class="elementor-button-text"><?php echo esc_html__( 'View and edit cart', 'woocommerce' ); // phpcs:ignore WordPress.WP.I18n ?></span>
		</a>
	</div>
	<?php
} // empty( $cart_items )
